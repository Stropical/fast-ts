/*
    Thanks to the TS2C repo for lots of these tests,
    they are well written and save tons of time!
*/
...
fs.readdir('./test/numbers', (err, files) => {
    if (err) {
        return console.log('Unable to scan directory: ' + err);
    } 
    //listing all files using forEach
    forEach(files,  function (file) {
        ...
        if(extension === '.ts') {
            console.log("Fast TS Compile: " + rawName)
            let fastts_cmd = 'node dist/main.js test/numbers/' + file + ' -o test/numbers/' + file;
    ...
    forEach(files,  function (file) {
        ...
        if(extension === '.ts') {
            console.log("Clang Compile: " + rawName)
            let clang_cmd = 'clang++ test/numbers/' + rawName + '.cpp lib/lib.o -o test/numbers/' + rawName + '.exe -I lib/include'
            shell.exec(clang_cmd);
        }        
    });
    ...
    forEach(files,  function (file) {
        ...
        if(extension === '.ts') {
            console.log("Running: " + rawName)
            let exec_cmd = '"test/numbers/' + rawName + '.exe" > test/numbers/' + file + '.out.txt'
            shell.exec(exec_cmd);
        }        
    });
})