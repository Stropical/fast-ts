    ...
    fs.readdir('./test/realworld', (err, files) => {
        forEach(files,  function (file) {
            ...
            if(extension === '.ts') {
                ...
                let expectOut = fs.readFileSync('./test/realworld/' + rawName + '.res.expect', 'utf8')
...
test('Numbers Folder', () => {
    fs.readdir('./test/numbers', (err, files) => {
        forEach(files,  function (file) {
            ...
            if(extension === '.ts') {
                let progOut = fs.readFileSync('./test/numbers/' + file + '.out.txt', 'utf8')
                let expectOut = fs.readFileSync('./test/numbers/' + rawName + '.res.expect', 'utf8')
                progOut = progOut.replace(/[\r]+/g, '');
                progOut = progOut.replace(/[\n]+/g, '\r\n');
